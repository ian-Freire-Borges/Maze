  ...
  export default function TruePlayerMove({ setScreen, setGameResult, maze, setMaze, setExitFound, exitFound, moveSpeed, isAutoMoving, cellDimensions, setScore,mazeRef}) {
    ...
    const tryMovePlayer = (currentPos) => {
      ...
      for (const { dx, dy, dir } of directions) {
        ...
        if (
          (!playerPanic.current && currentMaze[newY]?.[newX] === 0 && !visited.current.has(key)) ||
    ...
    const backtrack = () => {
      ...
      if (currentMaze[prevPos.y]?.[prevPos.x] === 4) {
        setGameResult(false);
        setExitFound(true);
    ...
    useEffect(() => {
      ...
      const moveInterval = setInterval(() => {
        ...
        if(currentMaze[playerPosition.y][playerPosition.x]===4){
          setExitFound(true);
          setGameResult(false);
